package com.praveenkesarwani.service;

import com.praveenkesarwani.exception.ParkingLotException;
import com.praveenkesarwani.model.Car;
import com.praveenkesarwani.model.ParkingLot;
import com.praveenkesarwani.model.Slot;
import com.praveenkesarwani.model.parking.strategy.ParkingStrategy;
...
public class ParkingLotService {
   ...
   * {@link ParkingLotException} if there is
   ...
   * decide the slot
   * number and then the car is parked into the {@link ParkingLot} into that slot
   * number.
   *
  ...
  /**
   * Gets all the slots in which a car with given driver's age is parked.
   *
   * @param age Driver's age to be searched.
   * @return All matching slots.
   */
  public List<Slot> getSlotsForDriversAge(final int age) {
    final List<Slot> occupiedSlots = getOccupiedSlots();
    return occupiedSlots.stream()
        .filter(slot -> slot.getParkedCar().getDriversAge() == age)
